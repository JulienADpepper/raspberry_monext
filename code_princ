...
def liste_des_erreurs(l) : #permet de ne récupérer que les cas qui nous interresse 
...
def allumage(l): #programme chargé de l'allumage de la led sur le raspberry
    ...
    if (len(l) != 0) and not(state) : #si la liste n'est pas vide et la LED est éteinte alors il y a des alertes critical donc on allume
...
def acquittement(l): #demande a l'utilisateur s'il veut acquitter les evennements et enrichie la liste des evenemments acquittés
...
acquittement(liste_des_erreurs(l)) #lance la procédure d'acquittement
